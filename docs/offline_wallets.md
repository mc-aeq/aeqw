...
```
aeqw --create --createwatchingonly
```
...
granted that aeqd and aeqw are installed and vendored dependencies 
are up to date with dep:

```
cd $GOPATH/src/github.com/aeqw/cmd/movefunds
go install
```
...
3. Open a terminal and change directory to where unspent.json is. Then, copy 
    config.json from $GOPATH/src/github.com/aeqw/cmd/movefunds to 
	this directory.
	```
	cp $GOPATH/src/github.com/aeqw/cmd/movefunds/config.json config.json
	```
...
6. Connect aeqw on the cold machine. Synchronize the addresses on this 
