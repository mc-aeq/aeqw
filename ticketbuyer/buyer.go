...
func (t *TicketPurchaser) Purchase(height int64) (*PurchaseStats, error) {
	...
	if !t.cfg.NoSpreadTicketPurchases && toBuyForBlock > 0 {
		...
		log.Debugf("Your average purchase price for tickets in the pool is %.2f AEQ", yourAvgTixPrice)
		log.Debugf("Available funds of %.2f AEQ can buy %.2f tickets, %.2f tickets per block",
		...
		log.Debugf("Redeemed ticket value expected is %.2f AEQ, buys %.2f tickets, %.2f%% more",
			redeemedFunds, tixToBuyWithRedeemedFunds, tixToBuyWithRedeemedFunds/tixCanBuy*100)
		log.Debugf("Stake reward expected is %.2f AEQ, buys %.2f tickets, %.2f%% more",
